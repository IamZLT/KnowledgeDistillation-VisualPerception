...
![image-20250428232442550](../Asserts/images/image-20250429140029427.png)
...
第l层LLM解码器处理的图像令牌、文本令牌和输出文本令牌集合分别记为 $S_I^l = \{t_{l,i}^I \mid \forall i \in I_I\}$，$S_T^l = \{t_{l,i}^T \mid \forall i \in I_T\}$，以及 $S_{OT}^l = \{t_{l,i}^{OT} \mid \forall i \in I_{OT}\}$，其中 $t_{l,i}^I, t_{l,i}^T, t_{l,i}^{OT} \in \mathbb{R}^d$ 表示第l层对应的第i个令牌的向量表示。
